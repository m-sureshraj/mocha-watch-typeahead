...
class Autocomplete extends Prompt {
  ...
  // renderOption(listItem, isFocused, isStart, isEnd) {
  //   const prefix = dim('›');
  //   const scrollIndicator = isStart ? ' ↑' : isEnd ? ' ↓' : '';
  //   const body = isFocused ? bgYellow().black(listItem) : dim(listItem);
  //
  ...
  renderOption({ label }, index) {
    const prefix = dim('›');
    const isFocused = this.focusedItemIndex === index;
  ...
  render() {
    ...
    // fixme: when there is a search, scrolling doesn't seems correct
    ...
    const suggestions = this.filteredList
      ...
      //     index + startIndex === endIndex - 1 && endIndex < this.list.length
      //   );
      // })
